...
| 3 | MoE expert-offload cache (run 35B-A3B in 16GB) | flags `--moe-expert-cache-size/-policy/-stats/--moe-prefetch`; env `OWNHUB_MOE_MADV_RANDOM` / `OWNHUB_MOE_PREFETCH` | **Strat A code-complete** | flags (`common/arg.cpp`,`common.h`); telemetry (`common/moe-stats.h`, tap `ffn_moe_topk`); Strat A complete: inc1 MADV_RANDOM + inc2 engine pager (`src/llama-model.cpp`,`include/llama.h`) + routing WILLNEED driver (`common/moe-stats.h`,`common.cpp`) + inc3 recency + cold-tail DONTNEED (`llama_ownhub_moe_step`, env `OWNHUB_MOE_EVICT_EVERY/AGE`). Default==stock. Next: inc3b MTP-draft router look-ahead; Strat B slot cache | aligns with upstream #20757; validate on 16GB HW |
